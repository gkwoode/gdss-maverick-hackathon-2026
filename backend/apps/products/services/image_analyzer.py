...
_SYSTEM_PROMPT = (
    ...
    "Your job is to read product images and extract structured data for "
    "an Item Master Database (IMDB). Be precise; only report what is "
    "clearly visible. Never hallucinate brand names or weights."
)

_EXTRACTION_PROMPT = """Analyze the product image and return a single JSON object with these exact keys:

{
  "barcode": "<EAN-13 / UPC-A / QR code value visible on label, or null>",
  "category_type": "<top-level category, e.g. Beverages, Snacks, Dairy, Personal Care, Household, Health>",
  "segment_type": "<sub-category, e.g. Carbonated Drinks, Potato Chips, Yogurt, Shampoo, Detergent>",
  "manufacturer": "<full legal name of the manufacturing company>",
  "brand": "<brand name as printed on the label>",
  "product_name": "<full product name including variant / flavour / size descriptor>",
  "weight": "<weight or volume with SI or imperial unit, e.g. 500ml, 200g, 1.5L, 12oz — include both if printed>",
  "packaging_type": "<packaging format: Bottle | Can | Box | Bag | Jar | Pouch | Tetra Pak | Tube | Sachet | Other>",
  "country_of_origin": "<country name as printed, e.g. Ghana, United Kingdom, USA>",
  "promotional_messages": "<verbatim marketing / promotional text visible on pack, comma-separated if multiple, or null>",
  "confidence": {
    "barcode": 0.0,
    "category_type": 0.0,
    "segment_type": 0.0,
...
- Do not guess; if a value is ambiguous set confidence ≤ 0.5.
"""

# ---------------------------------------------------------------------------
# Fields in the canonical IMDB order
# ---------------------------------------------------------------------------
IMDB_FIELDS = [
    "barcode",
    "category_type",
...
def _empty_result() -> dict[str, Any]:
    return {
        **{f: None for f in IMDB_FIELDS},
        "confidence": {f: 0.0 for f in IMDB_FIELDS},
        "method": "none",
    }
...
def _ocr_fallback(image_bytes: bytes) -> dict[str, Any]:
    """Very basic OCR fallback — extracts raw text and tries simple regex rules."""
    try:
        import pytesseract
    ...
    # Weight pattern: digits + optional dot + optional space + unit
    weight_match = re.search(
        r"\b(\d+(?:\.\d+)?\s*(?:ml|ML|l|L|g|G|kg|KG|oz|OZ|lb|LB))\b", text
    )
    if weight_match:
    ...
    # Country patterns
    country_match = re.search(
        r"(?:Made|Manufactured|Produced)\s+in\s+([A-Za-z ]{2,30})", text, re.IGNORECASE
    )
    if country_match:
        result["country_of_origin"] = country_match.group(1).strip()
        result["confidence"]["country_of_origin"] = 0.6
...
def analyze_image(image_bytes: bytes) -> dict[str, Any]:
    ...
    # Always try pyzbar for barcode — it's more reliable than VLM for barcodes
    if not result.get("barcode"):
        barcode = extract_barcode_pyzbar(processed)
        if barcode:
            result["barcode"] = barcode
            if "confidence" in result:
                result["confidence"]["barcode"] = 0.98

    # Ensure confidence dict always has all keys
    conf = result.get("confidence", {})
    for field in IMDB_FIELDS:
        if field not in conf:
            conf[field] = 0.0 if result.get(field) is None else 0.5
    result["confidence"] = conf
