...
class IMDBRecordViewSet(ModelViewSet):
    ...
    Extra actions:
      POST /api/products/analyze/           — upload image → extracted data preview
      POST /api/products/check_duplicates/  — check a candidate dict for duplicates
      GET  /api/products/export/            — download CSV or Excel
    """
    ...
    def get_queryset(self):
        ...
        if category := params.get("category"):
            qs = qs.filter(category_type__icontains=category)
        if barcode := params.get("barcode"):
            qs = qs.filter(barcode__icontains=barcode)
        if needs_review := params.get("needs_review"):
            qs = qs.filter(needs_review=(needs_review.lower() == "true"))
        if search := params.get("search"):
            ...
            ) | qs.filter(brand__icontains=search) | qs.filter(barcode__icontains=search)
    ...
    def analyze(self, request):
        """
        Upload a product image; returns extracted + validated IMDB data
        (not yet saved to the database — the user reviews and confirms).
        """
        serializer = AnalyzeImageSerializer(data=request.data)
        if not serializer.is_valid():
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    ...
    def export(self, request):
        ...
        if fmt == "excel":
            ...
            filename = "imdb_records.xlsx"
        else:
            content = export_csv(qs)
            content_type = "text/csv; charset=utf-8"
            filename = "imdb_records.csv"
