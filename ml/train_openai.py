...
Output files (written to ml/data/):
  train_finetune.jsonl   -- fine-tuning conversations (images as base64)
...
# Load env (OPENAI_API_KEY, etc.) from backend/.env
...
SYSTEM_PROMPT = (
    ...
    "  manufacturer -- company name after 'Manufactured by'/'Made by'/"
    "'Distributed by'/'Marketed by'/'Imported by'/'Packed by';"
    ...
    "  country      -- from 'Made in X'/'Manufactured in X'/'Packed in X'/"
    "'Product of X'; null if absent\n"
...
def build_conversation(entry: dict) -> dict:
    ...
    # Up to 4 images per product (cost control for fine-tuning)
    ...
    for rel_path in image_paths:
        ...
        if abs_path.exists():
            user_content.append({
                "type": "image_url",
                "image_url": {
                    "url": image_to_data_url(abs_path),
                    "detail": "low",
                },
            })
...
def write_jsonl(conversations: list[dict], path: Path) -> None:
    ...
    print(
        f"  Saved {len(conversations):3d} examples -> {rel}"
        f"  ({size_kb:.1f} KB)"
    )
...
def upload_and_finetune(
    ...
    print(
        ...
        f" print(c.fine_tuning.jobs.retrieve('{job.id}'))\""
    )
    print(
        f"\n  Once complete, set OPENAI_MODEL={job.fine_tuned_model}"
        " in backend/.env"
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Build OpenAI fine-tuning JSONL from train.json"
    )
    parser.add_argument(
        "--upload", action="store_true",
        help="Upload JSONL and start fine-tuning job",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--suffix", default="imdb-extractor",
        help="Fine-tuned model name suffix",
    )
    parser.add_argument(
        "--val-frac", type=float, default=0.1,
        help="Fraction of train set to use as validation",
    )
    ...
    print(
        f"      Fine-tune: {len(finetune_data)} | Validation: {len(val_data)}"
    )

    print("\n[2/3] Building JSONL (encoding images as base64) ...")
    ...
    for entry in finetune_data:
        ...
        if not available:
            skipped += 1
            continue
        train_convs.append(build_conversation({**entry, "images": available}))
    ...
    for entry in val_data:
        ...
        if not available:
            continue
        val_convs.append(build_conversation({**entry, "images": available}))

    if skipped:
        print(f"      [WARN] Skipped {skipped} products (no images found).")
    ...
    if args.upload:
        upload_and_finetune(
            train_jsonl, val_jsonl, args.model, args.epochs, args.suffix
        )
    else:
        print("\n  To upload and start fine-tuning, run:")
        print(
            f"    python ml/train_openai.py --upload"
